...
async def auto_filter(client, msg, spoll=False):
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}')] for file in files ]
        else:
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("I couldn't find anything related to that. try to ask in group !")
        await asyncio.sleep(8)
        await k.delete()
        return
